...
/**
 * RentedEquipmentBlock — блок "Арендованная техника" с кнопками операций
 */
...
<template>
  <UiCard class="equipment-block">
    <div class="equipment-block__header">
      <h3 class="equipment-block__title">Арендованная техника</h3>
    </div>

    <!-- Action buttons -->
    <div v-if="activeMode === 'none'" class="equipment-block__actions">
      <UiButton variant="primary" class="equipment-block__btn-new" @click="startCreate">
        Новая запись
      </UiButton>
      <div class="equipment-block__btn-row">
        <UiButton variant="secondary" class="equipment-block__btn-half" @click="startEdit">
          Редактирование
        </UiButton>
        <UiButton variant="secondary" class="equipment-block__btn-half" @click="startDelete">
          Удалить запись
        </UiButton>
      </div>
    </div>
    ...
    <div v-else-if="activeMode === 'delete' && foundShiftForDelete" class="equipment-block__form-area">
      <div class="equipment-block__found-record">
        ...
        <dl class="equipment-block__record-details">
          ...
          <div>
            <dt>Водитель:</dt>
            <dd>{{ currentEquipmentShift?.driver_name || '—' }}</dd>
          </div>
          <div>
            <dt>Дата:</dt>
