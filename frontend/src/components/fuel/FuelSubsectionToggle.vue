...
/**
 * FuelSubsectionToggle — слайдер-переключатель между подразделами потребления топлива
 *
 * Паттерн: аналогичен MileageSubsectionToggle.vue
 */
import { ref, computed, onMounted, onUnmounted, nextTick } from 'vue'

const props = defineProps<{
  modelValue: 'general' | 'by-driver'
}>()

const emit = defineEmits<{
  (e: 'update:modelValue', value: 'general' | 'by-driver'): void
}>()

const generalBtn = ref<HTMLButtonElement | null>(null)
const byDriverBtn = ref<HTMLButtonElement | null>(null)
...
const indicatorStyle = computed(() => {
  ...
  const activeBtn = props.modelValue === 'general' ? generalBtn.value : byDriverBtn.value
  const offsetBtn = props.modelValue === 'general' ? null : generalBtn.value
...
function selectOption(option: 'general' | 'by-driver') {
  ...
  <div class="subsection-toggle">
    <div ref="trackRef" class="subsection-toggle__track">
      ...
      <button
        ...
        :class="{ 'subsection-toggle__option--active': modelValue === 'general' }"
      ...
      <button
        ...
        type="button"
        class="subsection-toggle__option"
        :class="{ 'subsection-toggle__option--active': modelValue === 'by-driver' }"
        @click="selectOption('by-driver')"
      >
        По водителям
      </button>
    </div>
