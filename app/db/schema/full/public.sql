 ...
 WITH base AS (
         SELECT w.work_date,
           ...
           FROM (public.mv_work_actual_daily_value_rows w
             ...
             JOIN public.dim_unit du ON ((du.unit_id = vr.unit_id))
          WHERE ((w.work_status_id = 3) AND (vr.work_type_id = 3) AND (du.unit_name = ANY (ARRAY['1 км'::text, '1000 M2'::text, '10000 M2'::text])) AND (w.done_by_subcontractor = false))
 ...
 WITH base AS (
         SELECT w.work_date,
           ...
           FROM ((public.mv_work_actual_daily_value_rows w
             JOIN public.dim_work_item vr ON ((vr.work_item_id = w.work_item_id)))
             JOIN public.dim_unit du ON ((du.unit_id = vr.unit_id)))
          WHERE ((w.work_status_id = 3) AND (vr.work_type_id = 3) AND (du.unit_name = '1 км'::text) AND (w.done_by_subcontractor = false))
          GROUP BY w.work_date, w.work_item_id, w.road_section_id, du.unit_id
        )
 SELECT b.work_date,
    b.done_work_ids_arr AS done_work_ids,
